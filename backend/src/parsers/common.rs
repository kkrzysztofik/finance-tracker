...
pub fn parse_polish_decimal(s: &str) -> Result<Decimal, String> {
    let cleaned: String = s.trim().replace(['\u{a0}', ' '], "").replace(',', ".");
...
pub fn fix_mojibake(s: &str) -> String {
    ...
    if let Ok(latin1) = latin1_bytes {
        if let Ok(fixed) = String::from_utf8(latin1) {
            let valid_controls = fixed
                .chars()
                .all(|c| !c.is_control() || c == '\n' || c == '\r' || c == '\t');
            if fixed != text && valid_controls {
                return fixed;
            }
        }
    }
...
pub fn compute_hash(account: &str, date: &str, amount: &str, description: &str) -> String {
    let normalized_desc = description.trim().to_lowercase().replace("  ", " ");
...
mod tests {
    ...
    fn test_parse_polish_decimal() {
        assert_eq!(
            parse_polish_decimal("-1180,00").unwrap(),
            Decimal::from_str("-1180.00").unwrap()
        );
        assert_eq!(
            parse_polish_decimal("340,00").unwrap(),
            Decimal::from_str("340.00").unwrap()
        );
        assert_eq!(
            parse_polish_decimal("-49,47").unwrap(),
            Decimal::from_str("-49.47").unwrap()
        );
        assert_eq!(
            parse_polish_decimal("-7,19").unwrap(),
            Decimal::from_str("-7.19").unwrap()
        );
        assert_eq!(
            parse_polish_decimal("50").unwrap(),
            Decimal::from_str("50").unwrap()
        );
        assert_eq!(
            parse_polish_decimal("-11").unwrap(),
